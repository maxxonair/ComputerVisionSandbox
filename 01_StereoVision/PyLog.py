...
class PyLog:
    ...
    def __pLogCloseMsg( self ):
        ...
        strStats = "\n[---] "+str(self.iCountLogMsgs)+" logged messages.\n[WRN] "+str(self.iCountWarning)+" Warnings.\n[ERR] "+str(self.iCountError)+" Errors. \n"
        # Write to console 
        if self.flagIsConsolePrint:
            print()
            print(strLogFinal)
    ...
    def __init__( self, strLogFileName, flagIsConsolePrint, flagIsSaveLogFile):
        
        now = datetime.now()
        dt_string = now.strftime("%d_%m_%Y__%H_%M_%S")
        
        # Assign log file path
        self.strLogFilePath = dt_string + "__" + strLogFileName + ".txt"
    ...
    def pLogMsg( self, strLogMsg ):
        ...
        strLogFinal = self.__createLineNumber()+"[MSG] " + strLogMsg
        # Write to console 
        if self.flagIsConsolePrint:
            print(strLogFinal)
        # Write Log Message to file 
    ...
    def pLogErr( self, strLogMsg ):
        ...
        self.iCountLogMsgs = self.iCountLogMsgs + 1
        strLogFinal = self.__createLineNumber()+"[ERR] " + strLogMsg
        # Write to console 
        if self.flagIsConsolePrint:
            print(strLogFinal)
        # Write Log Message to file 
    ...
    def pLogWrn( self, strLogMsg ):
        ...
        self.iCountLogMsgs = self.iCountLogMsgs + 1
        strLogFinal = self.__createLineNumber()+"[WRN] " + strLogMsg
        # Write to console 
        if self.flagIsConsolePrint:
            print(strLogFinal)
        # Write Log Message to file 
