...
class MonoCalibrator:
  ...
  img_file_exts = [".png", ".jpeg", ".jpg", ".pgm"]
  ...
  def _createChessboardObjectPoints(self):
    self.objp = np.zeros(
        (self.boardSize[1] * self.boardSize[0], 3), np.float32)
  ...
  def calibrate(self):
    ...
    self.images = [p for p in Path(
        self.sInputFilePath).iterdir() if p.suffix in self.img_file_exts]

    if len(self.images) == 0:
      raise RuntimeError(
          f'No calibration images found at location {self.sInputFilePath}')
    ...
    for ii, fname in tqdm(enumerate(self.images)):
      # Load image
      self.log.pLogMsg(f'[{ii}] load image {fname}')
      img = cv.imread(fname)
      ...
      if patternFound == True:
        ...
        self.log.pLogMsg(str(imageIndex) +
  ...
  def recalibrate(self):
    ...
    if self.bFlagCalibrationComplete:
      ...
      self.log.pLogMsg("")
      self.log.pLogErr(
          'Re-calibration failed (CalibrationComplete flag is False)')
      self.log.pLogMsg("")
    ...
    for ii, imageData in enumerate(self.aImageList):
      if imageData.isPatternFound == True and imageData.averageReprojError > reprojErrThreshold:
        ...
        self.log.pLogMsg("Remove Image " + str(imageData.imageIndex) +
                         " (" + str(imageData.averageReprojError) + ")")
        counter = counter + 1
    ...
    for imageData in self.aImageList:
      if imageData.isPatternFound == True:
        ...
        self.log.pLogMsg('#' + str(imageData.imageIndex) + " " + sBarString
                         + ' - average: ' +
                           '{:.5f}'.format(imageData.averageReprojError)
                         + ' , min  ' +
                           '{:.5f}'.format(imageData.minReprojError)
                         + ' , max  ' +
                           '{:.5f}'.format(imageData.maxReprojError)
                         + ' , std  ' +
                           '{:.5f}'.format(imageData.stdReprojError)
                         )
  ...
  def saveResults(self):
    ...
    if self.bFlagCalibrationComplete:
      fileStorage.open(
          (self.sParameterFilePath + "calibration.yaml"), cv.FileStorage_WRITE)
  ...
  def calcReprojErrorArrayPerImage(self, imageData):
    ...
    imgpoints2, _ = cv.projectPoints(self.objp,
                                     ...
                                     self.aKmat,
                                     self.aDist)
    reprojErrorArray = np.absolute(
        np.array(imageData.imagePoints) - np.array(imgpoints2))
  ...
  def rectify(self):
    ...
    if self.bFlagCalibrationComplete:
      ...
      mapx, mapy = cv.initUndistortRectifyMap(self.aKmat,
                                              ...
                                              cv.CV_32FC1)
      self.log.pLogMsg(
          'Saving camera undistortion map -> cam_undistortion_map.tiff')
      (mapx, mapy) = cv.convertMaps(
          map1=mapx, map2=mapy, dstmap1type=cv.CV_16SC2)
      stacked = np.dstack([mapx.astype(np.uint16), mapy])
      cv.imwrite((Path(self.sParameterFilePath) /
                  "undistortion_map.tiff").absolute().as_posix(), stacked)

      for fname in tqdm(self.images):
        # Load image
        img = cv.imread(fname)
        h, w = img.shape[:2]
